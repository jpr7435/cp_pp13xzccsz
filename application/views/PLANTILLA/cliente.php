...
<section class="main-container">
    ...
    <input type="hidden" id="asesorAsignado" name="asesorAsignado" value="<?php echo $cliente[0]['idAsesor']; ?>"/>
    <input type="hidden" id="asesorActivo" name="asesorActivo" value="<?php echo $session['id']; ?>"/>
    <!-- Page header -->
    <div class="header bg-amarillo">
        <div class="header-content">
            <div class="page-title">
                <i class="icon icon-user position-left"></i> <?php echo $cliente[0]['nombre'] . " - " . $cliente[0]['documento'] . " - $" . number_format($cliente[0]['saldoPareto'], 0) . " - <span style='color: #2E2EFE;'>" . $mejor1[0]['descripcion'] . "</span><span id='prueba' style='cursor: pointer;'>....</span>"; ?>
                <?php
                if ($session['tarea'] != "") {
    ...
    <div class="container-fluid page-content">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-flat borde-azul" style="height: 130px; overflow-x: auto; overflow-y: hidden; width: auto !important;">
                    <div class="panel-body" style="height: 130px; overflow-x: auto; width: auto !important;">
                        ...
                        if ($mesEntero == 1) {
                            ...
                            $mesAnterior = $mesEntero - 1;
                            $dosMeses = $mesEntero - 2;
                        }

                        $mesinfo = $ci2->vista->getMesInfo($mesEntero, $session['proyecto_activo']);
                        $Antinfo = $ci2->vista->getMesInfo($mesAnterior, $session['proyecto_activo']);
                        $dosInfo = $ci2->vista->getMesInfo($dosMeses, $session['proyecto_activo']);

                        if ($mesEntero < 10) {
                            $mesEntero = "0" . $mesEntero;
                        }
                        ...
                        $dataestemes = $ci2->vista->getMesData($cliente[0]['documento'], $mesEntero, $session['proyecto_activo']);
                        $dataanterior = $ci2->vista->getMesData($cliente[0]['documento'], $mesAnterior, $session['proyecto_activo']);
                        $datados = $ci2->vista->getMesData($cliente[0]['documento'], $dosMeses, $session['proyecto_activo']);

                        $total = count($dataestemes) + count($dataanterior) + count($datados);

                        $ancho = $total * 200;
                        ?>

                        <ul style="width: <?php echo $ancho . "px"; ?>" class="customtimeline">
                            <li><span class="bg-red customtime-label"> <?php echo $mesinfo[0]['descripcion'] . " " . date("Y"); ?> </span></li>
                            <?php
                            foreach ($dataestemes as $este) {

                                $clase = "";
                                if ($este['idAccion'] == 1) {
                                    $contac = $ci2->vista->getContacto($este['idContacto'], $session['proyecto_activo']);
                                    $gr = $ci2->vista->getGruposContactoUno($contac[0]['idGrupo'], $session['proyecto_activo']);

                                    if ($gr[0]['idGrupo'] == 3) {
                                        $clase = 'class="icon icon-phone bg-green"';
                                        $event = "Llamada Efectiva";
                                    } else {
                                        $clase = 'class="icon icon-phone bg-blue"';
                                        $event = "Llamada No Efectiva";
                                    }

                                    if ($este['vlAcuerdo'] > 0) {
                                        $clase = 'class="icon icon-coin-dollar bg-yellow"';
                                        $event = "Gestion Productiva";
                                    }
                                } else if ($este['idAccion'] == 2) {
                                    $clase = 'class="icon icon-phone bg-green"';
                                    $event = "Llamada Entrante";
                                } else if ($este['idAccion'] == 4) {
                                    $clase = 'class="icon icon-home8 bg-red"';
                                    $event = "Pago Realizado";
                                } else if ($este['idAccion'] == 5) {
                                    $clase = 'class="icon icon-envelope bg-clearblue"';
                                    $event = "Pago Realizado";
                                } else {
                                    $clase = 'class="icon icon-plus22 bg-purple"';
                                    $event = "Pago Realizado";
                                }

                                $predia = explode(" ", $este['fechaGestion']);
                                $dia = explode("-", $predia[0]);

                                $userAction = $ci2->vista->getusuario($este['idAsesor']);
                                ?>

                                <li flag="<?php echo "A" . $este['idCallhist']; ?>" class="lineaTiempoAction">
                                    <i <?php echo $clase; ?>></i>
                                    <div class="customtimeline-description"><span class="customtimeline-date"><?php echo $dia[2]; ?></span></div>
                                    <div id="A<?php echo $este['idCallhist']; ?>" class="timeline-description">
                                        <span style="font-weight: bold; color: #424242;"><?php echo $userAction[0]['usuario']; ?></span></br>
                                        <span style="font-size: 11px; color: #424242;"> <?php echo $event; ?></span>
                                    </div>
                                </li>

                            <?php } ?>
                            <li><span class="bg-red customtime-label"> <?php echo $Antinfo[0]['descripcion'] . " " . date("Y"); ?> </span></li>
                            <?php
                            foreach ($dataanterior as $ant) {
                                $clase = "";
                                if ($ant['idAccion'] == 1) {
                                    $contac = $ci2->vista->getContacto($ant['idContacto'], $session['proyecto_activo']);
                                    $gr = $ci2->vista->getGruposContactoUno($contac[0]['idGrupo'], $session['proyecto_activo']);
                                ...
                                } else if ($ant['idAccion'] == 2) {
                                    $clase = 'class="icon icon-phone bg-green"';
                                    $event = "Llamada Entrante";
                                } else if ($ant['idAccion'] == 4) {
                                    $clase = 'class="icon icon-home8 bg-red"';
                                    $event = "Pago Realizado";
                                } else if ($ant['idAccion'] == 5) {
                                    $clase = 'class="icon icon-envelope bg-clearblue"';
                                    $event = "Pago Realizado";
                                } else {
                                    $clase = 'class="icon icon-plus22 bg-purple"';
                                ...
                                $predia = explode(" ", $ant['fechaGestion']);
                                $dia = explode("-", $predia[0]);

                                $userAction = $ci2->vista->getusuario($ant['idAsesor']);
                                ?>

                                <li flag="<?php echo "A" . $ant['idCallhist']; ?>" class="lineaTiempoAction">
                                    <i <?php echo $clase; ?>></i>
                                    <div class="customtimeline-description"><span class="customtimeline-date"><?php echo $dia[2]; ?></span></div>
                                    <div id="A<?php echo $ant['idCallhist']; ?>" class="timeline-description">
                                        <span style="font-weight: bold; color: #424242;"><?php echo $userAction[0]['usuario']; ?></span></br>
                                        <span style="font-size: 11px; color: #424242;"> <?php echo $event; ?></span>
                                    </div>
                                </li>
                            ...
                            <li><span class="bg-red customtime-label"> <?php echo $dosInfo[0]['descripcion'] . " " . date("Y"); ?> </span></li>
                            <?php
                            foreach ($datados as $dos) {
                                if ($dos['idAccion'] == 1) {
                                    $contac = $ci2->vista->getContacto($dos['idContacto'], $session['proyecto_activo']);
                                    $gr = $ci2->vista->getGruposContactoUno($contac[0]['idGrupo'], $session['proyecto_activo']);
                                ...
                                } else if ($dos['idAccion'] == 2) {
                                    $clase = 'class="icon icon-phone bg-green"';
                                    $event = "Llamada Entrante";
                                } else if ($dos['idAccion'] == 4) {
                                    $clase = 'class="icon icon-home8 bg-red"';
                                    $event = "Pago Realizado";
                                } else if ($dos['idAccion'] == 5) {
                                    $clase = 'class="icon icon-envelope bg-clearblue"';
                                    $event = "Pago Realizado";
                                } else {
                                    $clase = 'class="icon icon-plus22 bg-purple"';
                                ...
                                $predia = explode(" ", $dos['fechaGestion']);
                                $dia = explode("-", $predia[0]);

                                $userAction = $ci2->vista->getusuario($dos['idAsesor']);
                                ?>

                                <li flag="<?php echo "A" . $dos['idCallhist']; ?>" class="lineaTiempoAction">
                                    <i <?php echo $clase; ?>></i>
                                    <div class="customtimeline-description"><span class="customtimeline-date"><?php echo $dia[2]; ?></span></div>
                                    <div id="A<?php echo $dos['idCallhist']; ?>" class="timeline-description">
                                        <span style="font-weight: bold; color: #424242;"><?php echo $userAction[0]['usuario']; ?></span></br>
                                        <span style="font-size: 11px; color: #424242;"> <?php echo $event; ?></span>
                                    </div>
                                </li>
                            ...
                            <div class="tab-content">
                                ...
                                foreach ($creditos as $cr) {
                                    ...
                                    <div class="tab-pane <?php echo $active; ?>" id="<?php echo $cr['obligacion']; ?>">
                                        ...
                                        <?php if($cr['cancelado'] == 1){ ?>
                                            <div class="alert alert-danger alert-styled-left alert-bordered">
                                                Cuenta Cancelada
                                            </div>
                                        <?php } ?>
                                        <label>Obligacion: <?php echo $cr['obligacion']; ?></label>
                                        <div class="tabbable">
                                            <ul class="nav nav-tabs nav-tabs-highlight">
                    ...
                    <div class="panel-body" style="overflow-x: auto;">
                        ...
                        <div class="tabbable">
                            <ul class="nav nav-tabs nav-tabs-highlight">
                                ...
                                $flagtab = 0;
                                $prtemp = "";
                                foreach ($datavinculo as $dtpr) {
                                    if ($activo == 0) {
                                        $clases = 'class="active"';
                                    } else {
                                        $clases = '';
                                    }
                                    foreach ($dtpr as $dos2) {
                                        ?>
                                        <?php
                                        if (isset($dos2['obligacion'])) {

                                            if ($dos2['flag'] != $session['proyecto_activo']) {
                                                if ($prtemp != $dos2['flag']) {
                                                    $prtemp = $dos2['flag'];
                                                    ?>
                                                    <li <?php echo $clases; ?>><a href="#<?php echo $dos2['flag']; ?>" data-toggle="tab"><?php echo $dos2['flag']; ?></a></li>
                                                    <?php
                                                    $activo += 1;
                                                }
                                            }
                            ...
                            <div class="tab-content">
                                ...
                                foreach ($datavinculo as $dtpr2) {
                                    ...
                                    foreach ($dtpr2 as $dos) {
                                        $grillas = $this->vista->getGrillaValores($dos['flag']);
                                        if ($dos['flag'] != $session['proyecto_activo']) {
                                            ?>
                                            <div class="tab-pane <?php echo $active; ?>" id="<?php echo $dos['flag']; ?>">
                                                <div class="tabbable">
                                                    <?php if (isset($dos['obligacion'])) { ?>
                                                        <ul style="width: 100%; list-style: none;">
                                                            <li style="float: left; width: 10%;  margin-right: 4px; text-align: center;"><label style="font-weight: bold; font-size: 14px; background-color: #F78181; color: #FFF; width: 100%; text-align: center;">Obligacion</label></br><?php echo $dos['obligacion']; ?></li>
                                                            <?php
                                                            foreach ($grillas as $g2) {
                                                                if ($g2['ubicacion'] == "valores") {
                                                                    ?>
                                                                    <li style="float: left; width: 10%;  margin-right: 4px; text-align: center;"><label style="font-weight: bold; font-size: 14px; background-color: #F78181; color: #FFF; width: 100%; text-align: center;"><?php echo $g2['campo']; ?></label></br><?php echo number_format($dos[$g2['campo']], 2); ?></li>
                                                                    <?php
                                                                    $activoss += 1;
                                                                }
                                                            }
                                                            ?>
                                                            <li style="float: left; width: 10%;  margin-right: 4px; text-align: center;"><label style="font-weight: bold; font-size: 14px; background-color: #c39bd3; color: #FFF; width: 100%; text-align: center;">Saldo Actual</label></br><span style='background-color: #e5e8e8;'><?php echo number_format($dos['saldoActualizado'], 2); ?></span></li>
                ...
                <div class="panel-body">
                    <div class="panel-group accordion" id="accordion-styled">
                        <div class="panel">
                            ...
                            <div class="panel-heading">
                                <div class="panel-title">
                                    <a data-toggle="collapse" data-parent="#accordion-styled" href="#infoadicional">Informacion Adicional</a>
                                </div>
                            </div>
                            <div id="infoadicional" class="panel-collapse collapse">
                                <div class="panel-body" style="overflow-x: auto;">
                                    ...
                                    <div class="tabbable">
                                        <ul class="nav nav-tabs nav-tabs-highlight">
                                            <li class="active"><a href="#referencias" data-toggle="tab">Referencias</a></li>
                                            <li><a href="#trabajo" data-toggle="tab">Trabajo</a></li>
                                            <li><a href="#inmuebles" data-toggle="tab">Inmuebles</a></li>
                                            <li><a href="#vehiculos" data-toggle="tab">Vehiculos</a></li>
                                        </ul>

                                        <div class="tab-content">

                                            <div class="tab-pane active" id="referencias">
                                                <div class="form-group">
                                            ...
                                            <div class="tab-pane" id="trabajo">
                                                <div class="form-group">
                                                    <button class="btn btn-info" id="add-trabajo-btn" name="add-trabajo-btn">Agregar Trabajo</button>
                                                </div>
                                                <table class="table dataTable table-bordered">
                                                    <tr>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Lugar de Trabajo</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Direccion</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Tipo</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Telefono</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Comentario</th>
                                                    </tr>
                                                    <?php
                                                    foreach ($trabajo as $tra) {
                                                        ?>
                                                        <tr>
                                                            <td><?php echo $tra['lugar']; ?></td>
                                                            <td><?php echo $tra['direccion']; ?></td>
                                                            <td><?php echo $tra['tipo']; ?></td>
                                                            <td><?php echo $tra['telefono']; ?></td>
                                                            <td><?php echo $tra['comentario']; ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    ?>
                                            ...
                                            <div class="tab-pane" id="inmuebles">
                                                <table class="table dataTable table-bordered">
                                                    <tr>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Direccion</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Munucipio</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Departamento</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Matricula</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Estado</th>
                                                    </tr>
                                                    <?php
                                                    foreach ($inmuebles as $inmu) {
                                                        $municipio = $this->vista->getMunicipio($inmu['idMunicipio'], $session['proyecto_activo']);
                                                        $departamento = $this->vista->getusuario($inmu['idDepartamento'], $session['proyecto_activo']);
                                                        ?>
                                                        <tr>
                                                            <td><?php echo $inmu['direccion']; ?></td>
                                                            <td><?php echo $municipio[0]['municipio']; ?></td>
                                                            <td><?php echo $departamento[0]['departamento']; ?></td>
                                                            <td><?php echo $inmu['matricula']; ?></td>
                                                            <td><?php echo $inmu['estado']; ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    ?>
                                            ...
                                            <div class="tab-pane" id="vehiculos">
                                                <table class="table dataTable table-bordered">
                                                    <tr>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Marca</th>
                                                        <th style="background-color: #d5f5e3; font-size: 12px;">Modelo</th>
                                                    </tr>
                                                    <?php
                ...
                <div class="panel-body">
                    ...
                    <div class="panel-group accordion gestionAdelantada" id="accordion-styled">
                        ...
                        foreach ($gestion as $ges) {
                            ...
                            $dinaTotal = "";
                            foreach($dinamicos as $dina){
                                $dinaTotal .= "<small style='color: #2e86c1;'>".$dina['nombreCampo'].":&nbsp;&nbsp;".$ges[$dina['nombreCampo']]."</small>&nbsp;&nbsp;-&nbsp;&nbsp;";
                            }
                            ?>
                            ...
                            <div class="panel">
                                <div class="panel-heading" <?php echo $color; ?>>
                                    <div class="panel-title">
                                        <a <?php echo $class; ?> data-toggle="collapse" data-parent="#accordion-styled" href="#<?php echo $ges['idCallhist']; ?>"><?php echo $ges['fechaGestion'] . " - " . $user[0]['nombre'] . " - " . $acc . " " . $ges['telefono'] . " - " . $cont[0]['descripcion'] . " - " . $result[0]['descripcion'] . " - " . $fecacu . " - " . $vlacu. " </br> ".$dinaTotal; ?></a>
                                    </div>
                                </div>
                                <div id="<?php echo $ges['idCallhist'] ?>" class="panel-collapse collapse <?php echo $class2; ?>">
                                    <div class="panel-body">
                                        <p style="font-weight: bold;"><?php echo $ges['fechaGestion'] . " - " . $user[0]['nombre'] . " - " . $acc . " " . $ges['telefono'] . " - " . $cont[0]['descripcion'] . " - " . $result[0]['descripcion'] . " - " . $fecacu . " - " . $vlacu; ?></p>

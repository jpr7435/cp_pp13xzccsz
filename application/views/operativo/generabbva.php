...
$emails = $ci2->vista->getEmailsAll($session['proyecto_activo']);
$emailArr = array();

foreach($emails as $mailDb){
  $item = array(
    'documento' => $mailDb['documento'],
    'idEmail' => $mailDb['idEmail'],
    'email' => $mailDb['email'],
    'idActivo' => $mailDb['idActivo'],
    'agregado' => $mailDb['agregado']
  );
  array_push($emailArr, $item);
}

$gestionesArr = array();

foreach($gestionestotal as $gestDb){
  $item = array(
    'documento' => $gestDb['documento'],
    'idCallhist' => $gestDb['idCallhist'],
    'fechaGestion' => $gestDb['fechaGestion'],
    'hora' => $gestDb['hora'],
    'telefono' => $gestDb['telefono'],
    'idAccion' => $gestDb['idAccion'],
    'idContacto' => $gestDb['idContacto'],
    'idMotivo' => $gestDb['idMotivo'],
    'idResultado' => $gestDb['idResultado'],
    'ohacuerdo' => $gestDb['ohacuerdo'],
    'fechaAcuerdo' => $gestDb['fechaAcuerdo'],
    'vlAcuerdo' => $gestDb['vlAcuerdo'],
    'textoGestion' => $gestDb['textoGestion'],
    'idAsesor' => $gestDb['idAsesor']
  );
  array_push($gestionesArr, $item);
}

//$found_key = array_search('80470348', array_column($gestionesArr, 'documento'));


$clientesDB = $ci2->vista->getDataClientes($session['proyecto_activo']);
$clientesArr = array();

foreach($clientesDB as $cldb){
  $item = array(
    'documento' => $cldb['documento'],
    'nombre' => $cldb['nombre'],
    'idAsesor' => $cldb['idAsesor'],
    'mejorGestion' => $cldb['mejorGestion'],
    'ultimaGestion' => $cldb['ultimaGestion'],
    'FecUltimaGestion' => $cldb['FecUltimaGestion'],
    'activo' => $cldb['activo'],
    'mejorgestion180' => $cldb['mejorgestion180'],
    'fechamejorgestion180' => $cldb['fechamejorgestion180'],
    'mejorgestionmes' => $cldb['mejorgestionmes'],
    'intesidadmes' => $cldb['intesidadmes'],
    'estado_cliente_pago' => $cldb['estado_cliente_pago'],
    'fechapromesa' => $cldb['fechapromesa'],
    'estrategia' => $cldb['estrategia']
  );
  array_push($clientesArr, $item);
}
//print_r($clientesArr);
...
foreach ($gestiones as $g) {
  ...
  $found_key_mail = array_search($g['documento'], array_column($emailArr, 'documento'));

  $dataCl = null;
  $dataCl = array();
  $found_key = array_search($g['documento'], array_column($clientesArr, 'documento'));
  array_push($dataCl, $clientesArr[$found_key]);
  ...
  $found_key_callhist = array_keys(array_column($gestionesArr, 'documento'),$g['documento'],false);
  unset($gestCli);
  unset($call);
  $gestCli = array();
  $call = array();
  foreach($found_key_callhist as $key => $value){
    array_push($call, $gestionesArr[$value]);
  }
...
if(isset($call[0]['idResultado'])){
  ...
  foreach ($call as $cl) {
    ...
    if($flagTxt == 0){
      if(isset($emailArr[$found_key_mail]['email'])){
        $nuevo = $emailArr[$found_key_mail]['email'];
      }else{
        $nuevo = '';
      }
  ...
  $found_key_mejor = array_search($idMejor, array_column($gestionesArr, 'idCallhist'));
  unset($gestionMejor);
  $gestionMejor = array();

  //$gestionMejor = $ci2->vista->getCallId($idMejor, $session['proyecto_activo']);
  array_push($gestionMejor, $gestionesArr[$found_key_mejor]);
  ...
  /* Actividad Economica */
  if(isset($gestionMejor[0]['actividad'])){
